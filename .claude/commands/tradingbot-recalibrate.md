...
2. **Capture the pre-fit calibrator state** (so we can show the delta)
   Read `~/.tradingbot/calibration/p_bull_calibrator.json` if it exists. Save: mode, n_samples, brier_after, platt slope `a`. If the file is missing, this is the first fit — note that and continue.
...
3. **Run the backtest**
   ...
   ```
   Stream output. The script writes `~/.tradingbot/backtest/results_<timestamp>.json`.
...
4. **Refit the calibrator**
   ...
   ```
   The script reads the newest backtest result and writes `~/.tradingbot/calibration/p_bull_calibrator.json`.
...
## Notes
- Read-only on the codebase; only writes are `~/.tradingbot/backtest/results_*.json` and `~/.tradingbot/calibration/p_bull_calibrator.json`.
