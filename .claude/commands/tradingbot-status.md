...
Locate `~/.tradingbot/backtest/results_*.json` (newest by mtime). Compute age in hours.
...
### 5. Calibrator state
Load `~/.tradingbot/calibration/p_bull_calibrator.json` and report:
...
### 6. Open positions
Use `python -c "from tradingbot.execution.monitor import PositionMonitor; m = PositionMonitor(); print(m.position_count())"` — if a persisted state file exists, load it first; otherwise just report "no in-memory state".
