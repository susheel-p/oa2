...
# Phase 1 — Debaters
DEBATERS_ENABLED = _flag("TRADINGBOT_FLAG_DEBATERS", default=False)

# Phase 2 — Regime classifier
REGIME_CLASSIFIER_ENABLED = _flag("TRADINGBOT_FLAG_REGIME", default=False)

# Phase 3 — Consensus engine
CONSENSUS_ENGINE_ENABLED = _flag("TRADINGBOT_FLAG_CONSENSUS", default=False)
CONSENSUS_SHADOW_LOG = _flag("TRADINGBOT_FLAG_CONSENSUS_SHADOW", default=True)

# Phase 4 — Thompson bandit
BANDIT_ENABLED = _flag("TRADINGBOT_FLAG_BANDIT", default=False)
BANDIT_USE_POSTERIOR_MEAN = _flag("TRADINGBOT_FLAG_BANDIT_MEAN", default=True)

# Phase 5 — Dealer agent
DEALER_AGENT_ENABLED = _flag("TRADINGBOT_FLAG_DEALER", default=False)
DEALER_SHADOW_LOG = _flag("TRADINGBOT_FLAG_DEALER_SHADOW", default=True)

# Phase 8 — Event risk agent
EVENT_RISK_AGENT_ENABLED = _flag("TRADINGBOT_FLAG_EVENT_RISK", default=False)

# A/B comparison harness vs v1
AB_COMPARE_WITH_V1 = _flag("TRADINGBOT_FLAG_AB_V1", default=False)
...
EWMA_CORR_ENABLED = _flag("TRADINGBOT_FLAG_EWMA_CORR", default=True)

# Phase B — Sizing engine (oa2/sizing/: kelly.py, limits.py, cvar.py)
SIZING_ENGINE_ENABLED = _flag("TRADINGBOT_FLAG_SIZING", default=False)

# Phase C — Exit engine (oa2/execution/: exit.py, monitor.py, roll.py)
EXIT_ENGINE_ENABLED = _flag("TRADINGBOT_FLAG_EXIT", default=False)

# Phase D — Regime enhancements
SESSION_OVERLAY_ENABLED = _flag("TRADINGBOT_FLAG_SESSION", default=False)
CROSS_ASSET_REGIME_ENABLED = _flag("TRADINGBOT_FLAG_CROSS_ASSET", default=False)
...
FLOW_ADAPTER_ENABLED = _flag("TRADINGBOT_FLAG_FLOW_ADAPTER", default=False)
# E3: expiration-aware flow
EXPIRY_FLOW_ENABLED = _flag("TRADINGBOT_FLAG_EXPIRY_FLOW", default=False)
...
BACKTEST_ENABLED = _flag("TRADINGBOT_FLAG_BACKTEST", default=False)
