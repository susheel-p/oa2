---
name: oa2-paper-trade
description: Operates the oa2 automated paper trading system. Use when the user asks to run paper trading, start the daily scan, check paper trade results, review signals, push trade logs to GitHub, or manage the oa2 paper trading pipeline.
---

# oa2 Paper Trading Skill
...
- **Pipeline:** `oa2/graph/pipeline.py` — `run(ticker, account_size, book, monitor)`
- **Watchlist:** `oa2/watchlist/builder.py` — 22 tickers (WATCHLIST constant)
...
- **Default account size:** $50,000 (override with `--account-size` or `OA2_ACCOUNT_SIZE` env var)
...
- Market data is currently stub-based (yfinance delayed). Real-time data requires setting `OA2_FLOW_SOURCE` to a paid adapter.
